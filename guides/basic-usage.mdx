---
title: Basic usage
description: Learn fundamental patterns for using Vue components in Phoenix LiveView
---
...
For advanced component organization and custom resolution patterns, see [Configuration](/guides/configuration#component-organization).
...
<Tabs>
  <Tab title="useLiveVue() composable">
...
<Warning>
**Hooks inside slots are not supported**

Slots are rendered server-side and then sent to the client as a raw HTML. It happens outside of the LiveView lifecycle, so hooks inside slots are not supported.
...
- [Forms and validation](/guides/forms) for complex forms with server-side validation
