...
description: Common questions about LiveVue, including technical details, comparisons, and development tips
---

## General questions

<AccordionGroup>
  <Accordion title="Why LiveVue?">
  ...
  <Accordion title="What's with the name?">
...
<AccordionGroup>
  <Accordion title="How does LiveVue work?">
    ...
    <Note>
    Hooks fire only after `app.js` loads, which may cause slight delays in initial render.
    </Note>

    For a deeper dive into the architecture, see [Architecture](/guides/architecture).
  </Accordion>

  <Accordion title="What optimizations does LiveVue use?">
    LiveVue implements several performance optimizations:

    **1. Selective updates**

    Only changed props/handlers/slots are sent to client, achieved through careful `__changed__` assign modifications.
    ...
    **4. JSON patch diffing**
    ...
    Only changed props are sent over the WebSocket using JSON Patch format for minimal payloads.
  </Accordion>

  <Accordion title="What is the LiveVue.Encoder protocol?">
    The `LiveVue.Encoder` protocol is a crucial part of LiveVue's architecture that safely converts Elixir structs to maps before JSON serialization.
    ...
    **How to use it:**
    ...
    For complete implementation details including field selection, custom implementations, and third-party structs, see [Basic usage](/guides/basic-usage#custom-structs-as-props).
  ...
  <Accordion title="Why is SSR useful?">
    ...
    For complete SSR configuration, see [Configuration](/guides/configuration#server-side-rendering-ssr).
  </Accordion>

  <Accordion title="Can I nest LiveVue components inside each other?">
    ...
    **Workarounds:**
    ...
    2. **Standard Vue components:** You can nest standard (non-LiveVue) Vue components inside a LiveVue component. These child components are defined entirely within the parent's Vue template and do not have a corresponding `<.vue>` tag in LiveView.
...
<AccordionGroup>
  <Accordion title="How do I use TypeScript?">
    ...
    LiveVue uses Vite which provides built-in TypeScript support with no additional configuration needed.
  </Accordion>

  <Accordion title="Where should I put Vue files?">
    ...
    For advanced component organization, see [Configuration](/guides/configuration#component-organization).
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="How does LiveVue compare to LiveSvelte?">
    ...
    **Ecosystem:**
    ...
    For a detailed comparison with other solutions, see [Comparison](/guides/comparison).
  </Accordion>
</AccordionGroup>
